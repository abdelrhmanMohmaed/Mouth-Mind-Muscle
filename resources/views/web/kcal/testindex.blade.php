...
@section('main')
    ...
    {{-- @include('web.kcal.inc.selectPatientCalc') --}}
...
@dump($patient)
    ...
    @section('script')
        <script>
            //start get patientDetails and fadeOut modal
            $('body').on("click", "#submit", function() {
                var patient = $('#patient').val();
                $.ajax({
                    url: "{{ url('') }}/" + "get/patient/" + patient,
                    type: 'get',
                    dataType: 'json',
                    success: function(data) {
                        if (data == 'wrong') {
                            $('.patient_select').text('Sorry, Something is wrong please try again!!');
                        } else {
                            fireSuccessFun(data);
                        }
                    }
            ...
            //end get patientDetails and fadeOut modal
            ...
            //start fire success fun by response data
            function fireSuccessFun(data) {
                $('#input_macro_ideal_cho').val(data.macro.ideal_cho);
                $('#input_macro_ideal_protien').val(data.macro.ideal_protien);
                $('#input_macro_ideal_fat').val(data.macro.ideal_fat);
                $('#input_ideal_goal').val(data.ideal_goal);

                totalKcal('total_Kcal_cho', data.ideal_goal, data.macro.ideal_cho)
                totalKcal('total_Kcal_protien', data.ideal_goal, data.macro.ideal_protien)
                totalKcal('total_Kcal_fat', data.ideal_goal, data.macro.ideal_fat)


                totalGrams('total_grams_cho_first_modal', data.ideal_goal, data.macro.ideal_cho, 4)
                totalGrams('total_grams_protien_first_modal', data.ideal_goal, data.macro.ideal_protien, 4)
                totalGrams('total_grams_fat_first_modal', data.ideal_goal, data.macro.ideal_fat, 9)
                ...
                $('#title').text("To " + data.patient.first_name);
                $('.layOut').fadeOut(600);
                // $(".layOut").slideUp(1000);
                $(".total-distribution").toggleClass("modalActive");

            }
            //end fire success fun by response data
